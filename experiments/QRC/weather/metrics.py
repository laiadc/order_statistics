...
def lorentz_curves(
    ...
    *,
    ...
    for _ in tqdm(range(ns), desc="lorentz_curves", disable=not verbose):
        ...
        #p = np.array(
        #    [counts.get(format(k, f"0{nqbits}b"), 0) for k in range(D)],
        #    dtype=float,
        #) / nshots
...
def log_pk(x: float, k: int, D: int, eps: float = 1e-300) -> float:
    ...
    # Threshold where expm1 becomes unstable
    log_tail = np.log(-np.expm1(-a))
    if not np.isfinite(log_tail):
        log_tail = np.log(a)
        print('long tail', x, k, a, np.log(-np.expm1(-a)))

    return -k * a + (D - k) * log_tail

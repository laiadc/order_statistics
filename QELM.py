...
class QuantumCircQiskit:
    ...
    # Encoding
    # ─────────────────────────────────────────────────────────────────────
    def _encode(self, initial_state, qubit_offset=0, threshold = 1e-6):
        ...
        if self.encoding == 'quantum':
            ...
            # Initialize
            qc.append(StatePreparation(truncated), list(range(int(np.log2(len(state)))))) 
            return qc
    ...
    def run_circuit(self, initial_state):
        ...
        qc = self.get_qc(initial_state)
        self.estimator.options.default_shots = self.nshots
